    ...
    .of(
      yup.object().shape({
        ...
        colorSizeWiseQuantity: yup
          .object()
          .required("Size-wise quantity is required"),
...
const EditProduct = () => {
  ...
  const onSubmit = handleSubmit(async (formData) => {
    ...
    const productData = {
      ...
      ...(imgResData
        ? { images: imgResData.map((img) => ({ id: img.id })) }
        : {}),
    ...
    const { status: createStatus } = await fetchUpdateProduct(
      productId,
      productData,
    );
  ...
  if (isLoading) {
    return (
      <div className="pt-8 px-6">
        <div className="bg-content-bg px-5 py-6 rounded-xl border border-bc text-text text-sm">
  ...
  if (hasLoadError) {
    return (
      <div className="pt-8 px-6">
        <div className="bg-content-bg px-5 py-6 rounded-xl border border-red-300 text-red-600 text-sm">
          Failed to load this product. Please refresh and try again.
        </div>
      </div>
    );
  }
          ...
          <form onSubmit={onSubmit}>
            <div className="rounded-lg border border-bc bg-secondary px-4 py-3 mb-4">
              ...
              <p className="text-xs text-text opacity-70 mt-1">
                Update sizes first, then adjust color inventory. Total quantity
                is auto-calculated.
              </p>
            </div>
